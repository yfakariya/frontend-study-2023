...
Future<Issue?> getIssue({
  ...
  final uriPath = '/repos/$owner/$repository/issues/$issueNumber';
  final github = GitHub(auth: Authentication.withToken(token.accessToken));
  return github.getJSON<Map<String, dynamic>?, Issue?>(
    uriPath,
    convert: (s) => s == null ? null : Issue.fromJson(s),
  );
}

Future<String> getIssueBodyHtml({
  required AuthTokens token,
  required String owner,
  required String repository,
  ...
  final response = await github.request(
    'POST',
    '/markdown',
    headers: {'context': '$owner/$repository'},
    body: jsonEncode({'text': body}),
  );
