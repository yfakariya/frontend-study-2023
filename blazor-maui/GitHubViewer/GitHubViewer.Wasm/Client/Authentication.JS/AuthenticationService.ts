...
// based on Blazor's AuthenticationService.ts (https://github.com/dotnet/aspnetcore/blob/138388372bb7c6e35fdfa679da740fabc275a688/src/Components/WebAssembly/WebAssembly.Authentication/src/Interop/AuthenticationService.ts)
// MIT License (https://github.com/dotnet/aspnetcore/blob/3a6acd95c769bbbd2e5288d5844c81dee45fc958/LICENSE.txt)

// oidc-client-ts based re-limplementation to support non-OIDC OAuth2

import { SigninResponse, SignoutResponse, UserManager, UserManagerSettings, User, Log } from 'oidc-client-ts'
...
class OidcAuthorizeService implements AuthorizeService {
    ...
    private _disablesSilientSignIn: boolean;
    constructor(userManager: UserManager, disablesInitialSignIn: boolean, logger?: ManagedLogger) {
        this._userManager = userManager;
        this._disablesSilientSignIn = disablesInitialSignIn;
        this._logger = logger;
    }
    ...
    async signIn(context: AuthenticationContext) {
        ...
        if (!context.interactiveRequest) {
            ...
            try {
                ...
                if (this._disablesSilientSignIn) {
                    this.debug('Skip silent sign in');
    ...
    async signOut(context: AuthenticationContext) {
        ...
        try {
            ...
            await this._userManager.signoutRedirect(<any>this.createArguments(context.state, context.interactiveRequest));
            return this.redirect();
...
export class AuthenticationService {
    ...
    private static async initializeCore(settings?: UserManagerSettings & AuthorizeServiceSettings, logger?: ManagedLogger) {
        ...
        if (!settings && finalSettings && !logger && finalLogger) {
            if (finalLogger.debug || finalLogger.trace) {
                Log.setLogger(console);
            }

            if (finalLogger.trace) {
                finalLogger.log(LogLevel.Trace, 'initialize with: ' + JSON.stringify(settings));
            }
            ...
            if (window.parent !== window && !window.opener && (window.frameElement && userManager.settings.redirect_uri &&
                ...
                AuthenticationService.instance = new OidcAuthorizeService(userManager, /* disablesSilentSignIn */false, finalLogger);
            ...
            AuthenticationService.instance = new OidcAuthorizeService(userManager, settings.disablesSilentSignIn, logger);
